...
impl Service {
    pub async fn new(socket_path: &str) -> Result<Self> {
        let client = Client::from_path(socket_path)
            .await
            .with_context(|| format!("Failed to initialize Client, socket_path: {}", socket_path))?;
        Ok(Service {
    ...
    async fn prepare_snapshot(&self, cid: &str, ns: &str, img_name: &str) -> Result<(), anyhow::Error> {
        ...
        let _resp = self
            ...
            .await
            .with_context(|| format!("Failed to prepare snapshot, cid: {}, namespace: {}", cid, ns))?;
        Ok(())
    }
    ...
    pub async fn create_container(&self, image_name: &str, cid: &str, ns: &str) -> Result<(), Err> {
        ...
        self.prepare_snapshot(cid, ns, image_name)
            .await
            .with_context(|| format!("Failed to prepare snapshot, cid: {}, ns: {}, image: {}", cid, ns, image_name))?;
...
// TASK_UNKNOWN (0) — 未知状态
            ...
            cc.delete(with_namespace!(delete_request, namespace))
                .await
                .with_context(|| format!("Failed to delete container, cid: {}, ns: {}", cid, namespace))?;
    ...
    pub async fn create_and_start_task(
        ...
        self.create_task(cid, namespace, img_name)
            .await
            .with_context(|| format!("Failed to create task, cid: {}, ns: {}, img: {}", cid, ns, img_name))?;
    ...
    pub async fn delete_task(&self, cid: &str, ns: &str) -> Result<()> {
        ...
        let wait_result = timeout(time_out, async {
            ...
            c.wait(with_namespace!(wait_request, namespace))
                .await
                .with_context(|| format!("Failed to wait for task to finish, cid: {}, ns: {}", cid, namespace))?;
            Ok::<(), anyhow::Error>(())
        })
        .await;
        ...
        match wait_result {
            Ok(Ok(_)) => {
                ...
                c.delete(with_namespace!(req, namespace))
                    .await
                    .with_context(|| format!("Failed to delete task, cid: {}, ns: {}", cid, namespace))?;
            ...
            Err(_) => {
                ...
                c.kill(with_namespace!(kill_request, namespace))
                    .await
                    .with_context(|| format!("Force kill task failed, cid: {}, ns: {}", cid, namespace))?;
